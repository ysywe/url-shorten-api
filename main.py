...
def create_url(payload: schemas.URLCreate, db: DBSession) -> schemas.URLResponse:
...
def get_url(short_code: str, db: DBSession) -> schemas.URLResponse:
...
def update_url(short_code: str, payload: schemas.URLUpdate, db: DBSession) -> schemas.URLResponse:
    url = db.scalar(select(models.URLModel).where(models.URLModel.short_code == short_code))
...
def delete_url(short_code: str, db: DBSession):
    url = db.scalar(select(models.URLModel).where(models.URLModel.short_code == short_code))
...
def get_url_stats(short_code: str, db: DBSession) -> schemas.URLDetailsResponse:
    url = db.scalar(select(models.URLModel).where(models.URLModel.short_code == short_code))
...
def redirect_to_url(short_code: str, db: DBSession):
